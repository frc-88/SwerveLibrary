...
/**
 * Handles the mathematical processing of inverse kinematics for a swerve drive. All calculations
 * are robot-centric.
 */
public class InverseKinematics {
  ...
  public ModuleState[] calculate(VelocityState target) {
    ...
    if (target.isFieldCentric()) {
      throw new IllegalArgumentException(
          "Cannot give field-centric velocity state to inverse kinematics");
    }
    ...
    for (int idx = 0; idx < this.modules.length; ++idx) {
      ...
      if (combinedVector.getMagnitude() == 0) {
        ...
        moduleStates[idx] =
            new ModuleState(combinedVector.getAngle().asDouble(), combinedVector.getMagnitude());
      }
    }
  ...
  /**
   * Calculates the velocity vector for any module corresponding to the translation component of
   * motion.
   *
  ...
  protected Vector2D calculateModuleTranslationVector(VelocityState state) {
    ...
    return Vector2D.createPolarCoordinates(
        state.getTranslationSpeed(), new WrappedAngle(state.getTranslationDirection()));
  }
  ...
  protected Vector2D calculateModuleRotationVectors(VelocityState state, SwerveModule module) {
    ...
    Vector2D positionFromRotationCenter =
        location.plus(state.getCenterOfRotationVector().times(-1));
